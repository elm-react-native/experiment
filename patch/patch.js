...
module.exports = async (filename) => {
  ...
  const replacejsx = fs
    .readFileSync(p("./replace.jsx"), { encoding: "utf-8" })
    .split(/\r?\n/);
  const prependjsx = await util.promisify(fs.readFile)(p("./prepend.jsx"), {
    encoding: "utf-8",
  });
  const appendjsx = await util.promisify(fs.readFile)(p("./append.jsx"), {
    encoding: "utf-8",
  });
  const toRemove = [
  ...
  const source = (
    await util.promisify(fs.readFile)(filename, { encoding: "utf-8" })
  )
    .trim()
    .split(/\r?\n/);
  ...
  // await util.promisify(fs.writeFile)(p("./debug.jsx"), output);
  return output;
};
